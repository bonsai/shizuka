...
def format_usage(days: int = 14) -> str:
    ...
    grand = sum(d["t_in"] or 0 + d["t_out"] or 0 for d in daily)
    lines = [f"Token consumption — last {days} days  (total: {_fmt_si(grand)})"]

    # Daily sparkline
    if daily:
        totals = [d["t_in"] or 0 + d["t_out"] or 0 for d in daily]
        lines.append("")
        ...
        # Daily bars
        lines.append("")
        lines.append("Per day (in + out):")
        for d in daily:
            tot = (d["t_in"] or 0) + (d["t_out"] or 0)
            date = d["day"][5:] if len(d["day"]) == 10 else d["day"]
            lines.append(f"  {date}  {_hbar(tot, max_day)}  {_fmt_si(tot)}")

    # Per CLI
    if cli_stats:
        max_cli = cli_stats[0]["t_in"] + cli_stats[0]["t_out"] if cli_stats else 1
        lines.append("")
        lines.append("Per CLI:")
        for c in cli_stats:
            tot = (c["t_in"] or 0) + (c["t_out"] or 0)
            pct = tot / grand * 100 if grand else 0
            lines.append(f"  {c['cli']:<13}  {_hbar(tot, max_cli)}  {pct:5.1f}%  in:{_fmt_si(c['t_in'] or 0)}  out:{_fmt_si(c['t_out'] or 0)}")

    # Per provider
    if prov_stats:
        max_prov = (prov_stats[0]["t_in"] or 0) + (prov_stats[0]["t_out"] or 0) if prov_stats else 1
        lines.append("")
        lines.append("Per provider:")
        for p in prov_stats:
            tot = (p["t_in"] or 0) + (p["t_out"] or 0)
            pct = tot / grand * 100 if grand else 0
            lines.append(f"  {p['provider']:<22}  {_hbar(tot, max_prov)}  {pct:5.1f}%  {_fmt_si(tot)}")
